...
use rustc::middle::lang_items::{TransactionLockLangItem, TransactionUnlockLangItem};
...
fn make_mirror_unadjusted<'a, 'tcx>(
    ...
    let kind = match expr.kind {
        ...
        hir::ExprKind::Lock => {
            info!("[STM] mirroring my lock call");

            let def_id = cx.tcx.require_lang_item(TransactionLockLangItem, None);
            info!("[STM] def ID is {:?}", def_id);

            create_lock_call(cx, expr, def_id)
        }
        hir::ExprKind::Unlock => {
            info!("[STM] mirroring my unlock call");

            let def_id = cx.tcx.require_lang_item(TransactionUnlockLangItem, None);
            info!("[STM] def ID is {:?}", def_id);
...
/// Fill out the dummy lock/unlock call with a real lock/unlock call from the lang item.
fn create_lock_call<'a, 'tcx>(
    cx: &mut Cx<'a, 'tcx>,
    expr: &hir::Expr,
    def_id: DefId,
) -> ExprKind<'tcx> {
    ...
    // Insert all the arguments (can only support usize 1)
    let mut args: Vec<ExprRef<'_>> = vec![];

    let fn_sig_bound = ty.fn_sig(cx.tcx);

    fn_sig_bound.map_bound_ref(|fn_sig| {
        info!("[STM] fn sig is {:?}", fn_sig);
        info!("[STM] inputs are {:?}", fn_sig.inputs());
        for input_ty in fn_sig.inputs() {
            info!("[STM] input is {:?}", input_ty);
            let arg = Expr {
                ty: input_ty,
                temp_lifetime,
                span: expr.span,
                kind: ExprKind::Literal {
                    // STM - random constant for debugging
                    literal: ty::Const::from_usize(cx.tcx, 99),
                    user_ty: None,
                },
            };
            args.push(arg.to_ref());
        }
    });
    ...
    let literal_expr = Expr {
        ...
        temp_lifetime,
        span: expr.span,
        kind: ExprKind::Literal { literal: ty::Const::zero_sized(cx.tcx(), ty), user_ty: None },
    };

    let literal_scope = region::Scope { id: expr.hir_id.local_id, data: region::ScopeData::Node };
    ...
    let scope_expr = Expr {
        ...
        span: expr.span,
        kind: ExprKind::Scope {
            region_scope: literal_scope,
            value: literal_expr.to_ref(),
            lint_level: LintLevel::Explicit(expr.hir_id),
        },
    };

    let out = ExprKind::Call { ty: ty, fun: scope_expr.to_ref(), args, from_hir_call: true };
    info!("[STM] out is {:#?}", out);
    out
}
